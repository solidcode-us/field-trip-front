...
class Offers extends MY_Controller {
    ...
    function __construct() {
		...
		$thisIp= $this->input->ip_address();
		$googleMapAttrib= (unserialize(file_get_contents('http://www.geoplugin.net/php.gp?ip='.$thisIp)));

		 
		$geoplugin_latitude=$googleMapAttrib['geoplugin_latitude'];
		$geoplugin_longitude=$googleMapAttrib['geoplugin_longitude'];

		$this->session->set_flashdata('geoplugin_latitude',$geoplugin_latitude);
		$this->session->set_flashdata('geoplugin_longitude',$geoplugin_longitude);
    ...
    function index() {
		...
		$this->data['interest_id'] = (isset($userInterestData->id)) ? $userInterestData->id :"";
   ...
   public function ajax_member_update() {
	...
	function delete_card_information() {
	  ...
	  function map_markers() {
 		...
 		$geoplugin_latitude=$this->session->flashdata('geoplugin_latitude');
 		$geoplugin_longitude=$this->session->flashdata('geoplugin_longitude');

		$query = $this->db->query("select  
		(((acos(sin(('$geoplugin_latitude'*pi()/180)) * sin((`activity_lat`*pi()/180))+cos(('$geoplugin_latitude'*pi()/180)) 
		*cos((`activity_lat`*pi()/180))  *cos((('$geoplugin_longitude'- `activity_lon`)*pi()/180))))*180/pi())*60*1.1515) 
		AS distance, activity_title,activity_address,activity_lat,activity_lon,activity_status
		from activities having distance<25  and activity_status='approved' ;");
