...
class Ion_auth_model extends CI_Model
{
	...
	public function login_verify($user, $pass, $isReset=0)
		 {
		  if($this->ion_auth_model->login($user, $pass)) {
		  	if( $isReset){
		  		$this->session->set_flashdata('confirmation_message1', 'Your password was reset and you are now logged in. For future reference, your new password was sent to the email on file.');
		  		return true;
		  	}else{
		  		$this->session->set_flashdata('confirmation_message1', 'Thank you for activiating your account, you are now logged in to field-trip.com We have also sent you an email with your login detail for future use.');
		   		redirect('admin');		
		  	}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, password, active')
		                 // ->join('users_groups', 'users_groups.user_id=users.id')
						  ->where($this->identity_column, $this->db->escape_str($identity))->where('active', '1')
		                  ->limit(1)
		                  ->get($this->tables['users']);
